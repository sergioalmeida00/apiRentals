...
let CreateCarSpecificationUseCase = class CreateCarSpecificationUseCase {
    ...
    execute({ car_id, specification_id }) {
        return __awaiter(this, void 0, void 0, function* () {
            const carExists = yield this.carRespository.findById(car_id);
            if (!carExists) {
                throw new AppError_1.AppError("Car does not Exists!!");
            }
            const specifications = yield this.specificationRepository.findByIds(specification_id);
            carExists.specifications = specifications;
            yield this.carRespository.create(carExists);
            return carExists;
        });
    }
};
